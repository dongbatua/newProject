...
Slideshow.prototype.initUI = function(){
	...
	for(var i=0;i<this.imgs.length;i++){
		var str = "<img src='"+this.imgs[i]+"' style='width:"+this.width+";height:"+this.height+"px;position:absolute;left:0;top:0;z-index:"+(this.imgs.length-i)+";display:none'/>";
		$(this.boxId).append(str);
	}
	$(this.boxId+" img:first").css({"display":"block"});

	//2、所有按钮的创建
	var ulliStr = "<ul style='position:absolute;left:550px;bottom:0px;z-index:10;display:flex;justify-content:space-between'>";
	
	for(var i=0;i<this.imgs.length;i++){
		ulliStr +="<li ord='"+(i+1)+"' style='list-style:none;float:left;margin:10px;width:"+this.btnWidth+"px;height:"+this.btnHeight+"px;opacity:0.8;border-radius:50%;text-align:center;background-color:"+this.btnColor+";'></li>"
	}
	ulliStr +="</ul>";
	$(this.boxId).append(ulliStr);
	
	$(this.boxId+" ul li").click(function(){
